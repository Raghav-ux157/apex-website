...
const API = (() => {
  ...
  async function dispatchMock(method, path, body) {
    // Small micro-delay for realistic UI feedback
    ...
    if (cleanPath === '/orders') {
      ...
      if (method === 'POST') {
        ...
        const newOrder = {
          ...
          shipping_address: body.shipping ? `${body.shipping.address}, ${body.shipping.city} ${body.shipping.pincode}` : 'Atelier Direct',
  ...
  return {
    ...
    wishlist: {
      ...
      remove: (productId) => request('DELETE', `/wishlist/${productId}`),
    },
    ...
    orders: {
      ...
      requestReturn: (orderId, data) => request('POST', `/orders/${orderId}/return`, data),
    },
    ...
    admin: {
      ...
      returns: (params = {}) => {
        const qs = new URLSearchParams(params).toString();
        return request('GET', `/admin/returns${qs ? '?' + qs : ''}`);
      },
      updateReturn: (id, data) => request('PUT', `/admin/returns/${id}`, data),
